...
public class Diagrammer : CodeJunkieGenerator, IIncrementalGenerator {
    ...
    var options = context.AnalyzerConfigOptionsProvider
      .Select((options, _) => {
        ...
        return new GenerationOptions(
          StateChartsDiagramGeneratorDisabled: disabled
        );
      });
  ...
  public static bool IsStateChartCandidate(SyntaxNode node) =>
    ...
    classDeclaration.AttributeLists.SelectMany(list => list.Attributes)
      .Any(attribute =>
        attribute.Name.ToString() == Constants.STATE_CHART_ATTRIBUTE_NAME &&
  ...
  public StateChartImplementation? GetStateGraph(
    ClassDeclarationSyntax stateChartClassDecl,
    SemanticModel model,
  ...
  /// <summary>
  /// Looks at a state chart subclass, finds the state chart type in its
  /// inheritance hierarchy, and builds a state graph from it based on the
  /// state type given to the state chart type in the inheritance hierarchy.
  /// </summary>
  /// <param name="stateChartClassDecl">State chart class declaration.</param>
  /// <param name="model">Semantic model.</param>
  /// <param name="token">Cancellation token.</param>
  /// <returns>State chart graph.</returns>
  public StateChartImplementation? DiscoverStateGraph(
    ClassDeclarationSyntax stateChartClassDecl,
    SemanticModel model,
    CancellationToken token
  ) {
    ...
    var concreteState = (INamedTypeSymbol)symbol
      ...
      .FirstOrDefault(
        attr => attr.AttributeClass?.Name ==
          Constants.STATE_CHART_ATTRIBUTE_NAME_FULL
      )
      ?.ConstructorArguments[0]
      .Value!;

    // Search the state chart for all state subtypes, found recursively
    ...
    var getInitialStateMethod = symbol.GetMembers()
      .FirstOrDefault(
        member => member is IMethodSymbol method &&
          member.Name == Constants.STATE_CHART_GET_INITIAL_STATE
      );
  ...
  public IStateChartResult ConvertStateGraphToUml(
  ...
  public StateChartGraphData GetStateGraphData(
    ...
    var inputsBuilder = ImmutableDictionary
      .CreateBuilder<string, StateChartInput>();
    ...
    var outputsBuilder = ImmutableDictionary
      .CreateBuilder<IOutputContext, ImmutableHashSet<StateChartOutput>>();
    ...
    var handledInputInterfaces = type.AllInterfaces.Where(
      (interfaceType) => CodeService.GetNameFullyQualifiedWithoutGenerics(
        ...
        Constants.STATE_CHART_INPUT_INTERFACE_ID &&
