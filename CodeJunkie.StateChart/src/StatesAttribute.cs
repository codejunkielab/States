namespace CodeJunkie.StateChart;

using System;

/// <summary>
/// State chart attribute. Place this on a class that extends
/// <see cref="StateChart{TState}" /> to enable the StateCharts generators to
/// generate serialization utilities and/or a state diagram of the hierarchical
/// state machine that the states represents.
/// </summary>
[AttributeUsage(AttributeTargets.Class, AllowMultiple = false)]
public sealed class StateChartAttribute : Attribute {
  /// <summary>
  /// State type. If the state type is an interface, this should be the single
  /// base state record from which all other states are derived, not the
  /// interface.
  /// </summary>
  public Type StateType { get; }

  /// <summary>
  /// Whether or not a diagram should be generated for this states. The
  /// default is false.
  /// </summary>
  public bool Diagram { get; set; }

  /// <summary>
  /// State chart attribute. Place this on a class that extends
  /// <see cref="StateChart{TState}" /> to enable the StateCharts generators to
  /// generate serialization utilities and/or a state diagram of the
  /// hierarchical state machine that the states represents.
  /// </summary>
  /// <param name="stateType">State type. If the state is an interface,
  /// specify the single base state record from which all other states are
  /// derived, not the interface.</param>
  public StateChartAttribute(Type stateType) {
    StateType = stateType;
  }
}
